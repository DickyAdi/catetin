...
export function Testimonials() {
  return (
    <section className="mx-auto max-w-content px-lg pb-section">
      <h2 className="mb-xs text-center text-display-md">Kata Pengguna</h2>
      <p className="mb-xxl text-center text-body-small text-ink-faint">
      ...
      <div className="flex flex-wrap justify-center gap-lg">
        {TESTIMONIALS.map((testimonial) => (
          ...
          >
            ...
            <p className="mb-lg text-body text-ink">&ldquo;{testimonial.quote}&rdquo;</p>
            <div className="flex items-center gap-sm">
              <Avatar>
                <AvatarFallback>{testimonial.name.charAt(0)}</AvatarFallback>
              </Avatar>
              <div>
                <p className="text-body-small font-bold text-ink">{testimonial.name}</p>
                <p className="text-body-small text-ink-muted">{testimonial.place}</p>
              </div>
            </div>
          </div>
        ))}
