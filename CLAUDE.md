...
| Migrations | Alembic, a **runtime** dep of `apps/api` (it has to survive the `--no-dev` image build so the one-off `migrate` compose service can run it). **Never imported in `main.py`** — run `alembic upgrade head` in deploy, app only VERIFIES revision at startup (`version_check`) |
...
- **Containerized migrations are a separate, opt-in step.** Every compose file has a one-off `migrate` service (same image/env/volume as `api`, `command: alembic upgrade head`) behind `profiles: ["migrate"]`, so a plain `up` never touches the schema. Hence four targets: `infra-dev-up-migrate` / `infra-dev-up` and `infra-up-migrate` / `infra-up`. On a fresh volume the non-migrate variants are *expected* to fail `version_check` — that is the signal to use the `-migrate` one, not a bug.
- **The image's ignore file is `apps/api/Dockerfile.dockerignore`, not `apps/api/.dockerignore`.** Build context is the repo root, so BuildKit only honors the `<dockerfile path>.dockerignore` form. Under the wrong name it is silently ignored and the build ships `.mypy_cache`, `tests/`, `.env` and a stale `catetin.db`. Cache/venv patterns also need a `**/` prefix — they live under `apps/api/`, not at the context root.
- **The runtime image `mkdir`s `/data` owned by `appuser`.** Docker seeds a fresh named volume's ownership from the image directory under the mount point; without that line the volume comes up `root:root:755`, the non-root container cannot write it, and both alembic and the app fail with `unable to open database file` (which `version_check` then reports as the misleading "alembic_version table missing").
